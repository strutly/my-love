...
public class QiNiuController {
    ...
    public DataResult<FileRespVO> uploadImg(@RequestParam("file") MultipartFile file){
    ...
    @PostMapping("/file")
    public DataResult<FileRespVO> uploadCover(@RequestParam("file") MultipartFile file) {
        String fileName = file.getOriginalFilename();
        FileInputStream inputStream = null;
        try {
            inputStream = (FileInputStream) file.getInputStream();
        ...
        return uploadQNFile(inputStream, fileName);
    }
     ...
     * 将文件上传到七牛云
     */
    private DataResult<FileRespVO> uploadQNFile(FileInputStream file, String fileName) {
        DataResult<FileRespVO> result = DataResult.success();
        ...
        try {
            ...
            String upToken = auth.uploadToken(constantQiniu.getBucket());
            try {
                String key = "mylove/" + UUID.randomUUID().toString() + fileType;
